...
err_t add_line_to_graphic_scene(add_line_to_graphic_scene_params params)
{
    ...
    if (return_code == SUCCESS)
        return_code = get_y_of_point(params.end, &buffer_end_x);

    if (return_code == SUCCESS)
        return_code = get_y_of_point(params.end, &buffer_end_y);

    if (return_code == SUCCESS)
        return_code = get_global_window_size_params_width(&window_height, &window_width);

    if (return_code == SUCCESS)
        params.scene->addLine(buffer_start_x, window_height - buffer_start_y,
                       buffer_end_x, window_height - buffer_end_y, params.pen);
